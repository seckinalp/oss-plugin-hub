...
const SCORECARD_DIR = fs.existsSync(path.join(DATA_DIR, 'scorecard', 'scorecard-local'))
  ? path.join(DATA_DIR, 'scorecard', 'scorecard-local')
  : path.join(PROJECT_ROOT, 'scorecard-local');
const OUTPUT_FILE = fs.existsSync(path.join(DATA_DIR, 'scorecard'))
  ? path.join(DATA_DIR, 'scorecard', 'scorecard-platform-summary.json')
  : path.join(DATA_DIR, 'scorecard-platform-summary.json');
const PLATFORM_ROOT = fs.existsSync(path.join(DATA_DIR, 'platforms'))
  ? path.join(DATA_DIR, 'platforms')
  : DATA_DIR;
const TOP100_DIR_LABEL = fs.existsSync(path.join(DATA_DIR, 'platforms'))
  ? 'data/platforms/*/top100.json'
  : 'data/*/top100.json';
...
function getTop100Platforms() {
  if (!fs.existsSync(PLATFORM_ROOT)) return [];
  const entries = fs.readdirSync(PLATFORM_ROOT, { withFileTypes: true });
  const platforms = [];
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const platform = entry.name;
    const filePath = path.join(PLATFORM_ROOT, platform, 'top100.json');
...
function main() {
  ...
  const output = {
    ...
    source: {
      scorecardDir: normalizePath(path.relative(PROJECT_ROOT, SCORECARD_DIR)) || 'scorecard-local',
      top100Dir: TOP100_DIR_LABEL,
