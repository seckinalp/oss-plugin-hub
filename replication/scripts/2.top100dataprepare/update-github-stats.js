...
const PR_MAX_PAGES = parseInt(process.env.PR_MAX_PAGES || '5', 10);
const PR_SLEEP_MS = Math.round(parseFloat(process.env.PR_SLEEP || '0.5') * 1000);
const PR_CACHE_FILE = path.join(DATA_DIR, 'pr-friction-cache.json');
...
let prFrictionCache = loadPrFrictionCache();
...
function loadPrFrictionCache() {
  if (!fs.existsSync(PR_CACHE_FILE)) {
    return {};
  }
  try {
    return JSON.parse(fs.readFileSync(PR_CACHE_FILE, 'utf8'));
  } catch {
    return {};
  }
}

function savePrFrictionCache(cache) {
  fs.writeFileSync(PR_CACHE_FILE, JSON.stringify(cache, null, 2));
}
...
/**
 * Fetch PR friction (median external PR turnaround time in days)
 */
async function fetchPrFriction(owner, repo) {
  const durations = [];
  for (let page = 1; page <= PR_MAX_PAGES; page++) {
    const url = `${GITHUB_API}/repos/${owner}/${repo}/pulls?state=closed&sort=created&direction=desc&per_page=100&page=${page}`;
    const data = await fetchGitHub(url);
    ...
    if (PR_SLEEP_MS > 0) {
      await delay(PR_SLEEP_MS);
    }
  }

  if (!durations.length) {
    return { status: 'no-data', medianDays: null, count: 0 };
  }

  const med = median(durations);
  return { status: 'ok', medianDays: Math.round(med * 100) / 100, count: durations.length };
}

async function getPrFriction(owner, repo) {
  const key = `${owner}/${repo}`;
  if (!FORCE_REFETCH && prFrictionCache[key]) {
    return prFrictionCache[key];
  }

  try {
    const result = await fetchPrFriction(owner, repo);
    prFrictionCache[key] = result;
    savePrFrictionCache(prFrictionCache);
    return result;
  } catch (error) {
...
async function fetchRepoStats(owner, repo) {
  try {
    ...
    await delay(DELAY_MS);
    const prFriction = await getPrFriction(owner, repo);
    await delay(DELAY_MS);
...
function loadPlatformData(platform) {
  const platformDir = path.join(PLATFORM_ROOT, platform);
  ...
  const pluginsData = fs.existsSync(pluginsFile)
    ? JSON.parse(fs.readFileSync(pluginsFile, 'utf8'))
    : null;
  const top100Data = fs.existsSync(top100File)
    ? JSON.parse(fs.readFileSync(top100File, 'utf8'))
    : null;

  if (!pluginsData && !top100Data) {
    return null;
  }
 ...
 * Save platform data
 */
function savePlatformData(platform, platformData, stats) {
  const platformDir = path.join(PLATFORM_ROOT, platform);
  const pluginsFile = platformData.pluginsFile || path.join(platformDir, 'plugins.json');
  const top100File = platformData.top100File || path.join(platformDir, 'top100.json');
  const metadataFile = platformData.metadataFile || path.join(platformDir, 'metadata.json');

  const plugins = platformData.plugins || [];
  const top100 = platformData.top100 || [];
  ...
  if (platformData.pluginsData && pluginsFile) {
    platformData.pluginsData.plugins = plugins;
    fs.writeFileSync(pluginsFile, JSON.stringify(platformData.pluginsData, null, 2));
  } else if (pluginsFile && plugins.length) {
    fs.writeFileSync(pluginsFile, JSON.stringify({ plugins }, null, 2));
  }

  if (platformData.top100Data && top100File) {
    platformData.top100Data.top100 = top100;
    fs.writeFileSync(top100File, JSON.stringify(platformData.top100Data, null, 2));
  } else if (top100File && top100.length) {
    fs.writeFileSync(top100File, JSON.stringify({ top100 }, null, 2));
  }
...
async function main() {
  ...
  // Collect top100 plugins per platform
  const allPlugins = [];
  for (const platform in platformData) {
    const data = platformData[platform];
    const top100 = data.top100 && data.top100.length
      ? data.top100
      : data.plugins.filter(p => p.isTop100 === true);
    allPlugins.push(...top100.map(plugin => ({ platform, plugin })));
  ...
  const pluginsNeedingFetch = FORCE_REFETCH 
    ? allPlugins.length 
    : allPlugins.filter(p => !p.plugin.githubStats).length;
  const pluginsWithStats = allPlugins.filter(p => p.plugin.githubStats).length;
  ...
  for (let i = 0; i < allPlugins.length; i++) {
    const { platform, plugin } = allPlugins[i];
    ...
    try {
      ...
      if (stats) {
        ...
        const match = platformPlugins.find(p =>
          (p.id && plugin.id && p.id === plugin.id) ||
          (p.repo && plugin.repo && p.repo === plugin.repo)
        );
        if (match) {
          match.githubStats = stats;
        }
  ...
  for (const platform in platformData) {
    savePlatformData(platform, platformData[platform], pluginStats);
    console.log(`   Saved ${platform} data`);
  }
