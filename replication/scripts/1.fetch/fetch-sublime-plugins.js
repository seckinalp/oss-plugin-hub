...
const INSTALLS_MODE = (process.env.SUBLIME_INSTALLS_MODE || 'popular').toLowerCase();
const INSTALLS_PAGES = Number.isFinite(parseInt(process.env.SUBLIME_INSTALLS_PAGES, 10))
  ? parseInt(process.env.SUBLIME_INSTALLS_PAGES, 10)
  : 10;
const INSTALLS_DELAY_MS = Number.isFinite(parseInt(process.env.SUBLIME_INSTALLS_DELAY_MS, 10))
  ? parseInt(process.env.SUBLIME_INSTALLS_DELAY_MS, 10)
  : 200;
...
/**
 * Fetch popular install counts from Package Control
 */
...
async function loadPopularInstallMap(pages) {
  const installMap = new Map();
  for (let page = 1; page <= pages; page++) {
    console.log(`   - Fetching installs page ${page}/${pages}...`);
    const data = await fetchPopularPackages(page);
    if (data.packages) {
      data.packages.forEach(pkg => {
        installMap.set(pkg.name.toLowerCase(), pkg.unique_installs || 0);
      });
    }
    await new Promise(resolve => setTimeout(resolve, INSTALLS_DELAY_MS));
  }
  return installMap;
}
 ...
 */
function parseSublimePackage(packageName, packageData, installMap) {
  ...
  let downloads = packageData.downloads || 0;
  if (installMap && installMap.size > 0) {
    const byPackage = installMap.get(packageName.toLowerCase());
    const byName = installMap.get(name.toLowerCase());
    const installs = byPackage ?? byName ?? downloads;
  ...
  return {
    ...
    downloads: downloads,
    downloadStatsUpdated: downloads > 0 && installMap ? new Date().toISOString() : null,
...
async function main() {
  ...
  console.log(`   - Install stats mode: ${INSTALLS_MODE}`);
  if (INSTALLS_MODE === 'popular') {
    console.log(`   - Install stats pages: ${INSTALLS_PAGES}`);
  }
  if (INSTALLS_MODE === 'none') {
    console.log('   - Install stats disabled (use update-sublime-installs.js for counts)\n');
  } else {
    console.log('   - Set SUBLIME_INSTALLS_MODE=none to skip install stats\n');
  }
  try {
    ...
    let installMap = null;
    if (INSTALLS_MODE === 'popular') {
      console.log('?? Fetching popular install counts...');
      installMap = await loadPopularInstallMap(INSTALLS_PAGES);
      console.log(`? Fetched install counts for ${installMap.size} packages\n`);
    } else if (INSTALLS_MODE !== 'none') {
      console.log(`?? Unknown install stats mode "${INSTALLS_MODE}", skipping installs fetch\n`);
    }

    // Parse to our format
    console.log('🔄 Parsing packages...');
    const allParsedPlugins = allPackages.map(packageData =>
      parseSublimePackage(packageData.name, packageData, installMap)
    );
