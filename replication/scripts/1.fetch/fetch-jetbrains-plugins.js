...
const MAX_PAGES = Number.isFinite(parseInt(process.env.JETBRAINS_MAX_PAGES, 10))
  ? parseInt(process.env.JETBRAINS_MAX_PAGES, 10)
  : null;
// Default to include all plugins. Set JETBRAINS_GITHUB_ONLY=true to restrict to plugins with GitHub repos.
const GITHUB_ONLY = process.env.JETBRAINS_GITHUB_ONLY === 'true';
 ...
 */
async function fetchAllJetBrainsPlugins(pageSize = 20, maxPages = null) {
  const allPlugins = [];
  let page = 0;
  let totalFetched = 0;
  ...
  while (true) {
    if (maxPages !== null && page >= maxPages) {
      console.log(`   Reached max pages (${maxPages}), stopping early`);
      break;
    }
...
async function main() {
  ...
  console.log(`   - Set JETBRAINS_GITHUB_ONLY=true to limit to plugins with GitHub repos\n`);
  ...
  try {
    ...
    const rawPlugins = await fetchAllJetBrainsPlugins(PAGE_SIZE, MAX_PAGES);
