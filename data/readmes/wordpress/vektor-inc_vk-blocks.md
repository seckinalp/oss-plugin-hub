...
例えばWordPressのログイン画面からの��作テストを作る場合は以下のようになります。
```
...
[ prefix ]_[ ���ロック名 ]_[ divのクラス名 ]-[ 属性名 ]-[ 属性値 ]
```
...
イレギュラー : 管理画面側での指定はなく、CSSでブレイクポイントが一つの場合（モ��イル/PC など）以下の設定がありえる
...
### vk-blocks-pro/develop バージョン変更・��訳
...
修正して再度プルリクになるが、ここでの不具合修��はもともと「決定済みの期待する動作」の修正なので、プラグインに記載のバージョン変更の必要はない。
...
1. [ ★ ] readme.txt の stable ��バージョンを上げてコミット
